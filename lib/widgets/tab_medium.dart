...
import 'package:news_app/tabs/category_tab5.dart';
...
class _TabMediumState extends State<TabMedium> {
  ...
  void _scrollListener() {
      final db = context.read<PopularBloc>();
      ...
      if (sb.tabIndex == 0) {
        ...
        if (!cb5.isLoading) {
          if (this.widget.sc!.offset >= this.widget.sc!.position.maxScrollExtent && !this.widget.sc!.position.outOfRange) {
            ...
            cb5.setLoading(true);
            cb5.getData(mounted, Config().initialCategories[4],);
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    return TabBarView(
      children: <Widget>[
        ...
        CategoryTab5(
          category: Config().initialCategories[4],
        ),
        Categories(true),
      ],
